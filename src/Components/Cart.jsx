                ...
                <div className='flex flex-col md:flex-row gap-5'>
                    ...
                    <div className='flex-grow border border-black/10 rounded-3xl p-[14px]'>
                        {cart.cart_Items.map((item) => (
                            ...
                            <div key={item.product.id} className='flex gap-4 my-4 border-b border-black/10 pb-4 last:border-0 last:pb-0'>
                                <div className='bg-[#F2F0F1] rounded-2xl min-w-[99px] min-h-[99px] flex justify-center items-center'>
                                    <Link to={`/products/${item.product.id}`}><img src={item.product.image} alt="" className='w-20 h-20 object-contain' />
                                    </Link>
                                </div>
                                ...
                                <div className='flex flex-col justify-between w-full'>
                                    ...
                                    <div className='flex justify-between items-start'>
                                        <div>
                                            ...
                                            <p className='font-sans text-[12px] '>Size:</p>
                                            <p className='font-sans text-[12px] '>Colour:</p>
                                        </div>
                                        <button className='cursor-pointer' onClick={() => handleDelete(item.product.id)}>
                                    ...
                                    <div className='flex justify-between items-center mt-2'>
                                        ...
                                        <div className='flex gap-3 bg-[#F0F0F0] w-[105px] min-h-[31px] justify-between items-center px-3 rounded-full'>
                                            <button className='cursor-pointer' onClick={() => handleQuantityChange(item.product.id, -1)}><img src={Minus} alt="Decrease" /></button>
                                            <span className='font-bold text-[14px]'>{item.quantity}</span>
                                            <button className='cursor-pointer' onClick={() => handleQuantityChange(item.product.id, 1)}><img src={Plus} alt="Increase" /></button>
                                        </div>
                                    </div>
                    ...
                    <div className='border border-black/10 rounded-3xl p-[20px] font-sans w-full md:w-[400px] h-fit'>
                        ...
                        <div className='flex justify-between py-[16px] gap-2'>
                            ...
                            <button className='bg-black text-[14px] text-white py-[12px] px-[20px] rounded-full cursor-pointer '>
                                Apply
                            </button>
                        </div>
                        <div className='mt-2'>
