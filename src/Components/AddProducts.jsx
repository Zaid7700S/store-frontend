...
const AddProduct = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const newProductId = response.data.id; // Get the generated ID

            // 2. If an image was selected, upload it to the new R2 endpoint
            if (imageFile && newProductId) {
                const formData = new FormData();
                formData.append("image", imageFile);

                await api.post(`/api/Products/${newProductId}/upload-image`, formData, {
                    headers: {
            ...
            console.error(error);
            setMessage("Failed to add product. Please check your connection.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-[70vh] flex justify-center items-center px-4 py-10 font-sans">
            <div className="w-full max-w-2xl border border-black/10 rounded-3xl p-8 shadow-sm bg-white">
                ...
                <p className="text-black/60 mb-6">
                    Enter the details below to create a new product in the store.
                </p>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name='price' value={productData.price} onChange={handleChange} placeholder='0.00' className="w-full border border-black/20 rounded-xl p-3 outline-none focus:border-black/50" required />
                    </div>

                    <div>
                        <label className="block font-medium mb-2 text-sm">Quantity</label>
                        <input type="number" name='quantity' value={productData.quantity} onChange={handleChange} placeholder='0' className="w-full border border-black/20 rounded-xl p-3 outline-none focus:border-black/50" required />
                    </div>

                    <div>
                        <label className="block font-medium mb-2 text-sm">Rating (0-5)</label>
                        <input type="number" step="0.1" max="5" name='rating' value={productData.rating} onChange={handleChange} placeholder='0.0' className="w-full border border-black/20 rounded-xl p-3 outline-none focus:border-black/50" />
                    </div>

                    <div>
                        <label className="block font-medium mb-2 text-sm">Category</label>
                        <input type="text" name='category' value={productData.category} onChange={handleChange} placeholder='e.g. Men, Gym, Casual' className="w-full border border-black/20 rounded-xl p-3 outline-none focus:border-black/50" required />
                    </div>
