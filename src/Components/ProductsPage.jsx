...
const ProductsPage = () => {
    // Read the search query from the URL (powered by the Navbar)
    ...
    // Extract unique categories dynamically (Splitting comma-separated tags)
    const categories = useMemo(() => {
        ...
        products.forEach(p => {
            if (p.category) {
                ...
                const tags = p.category.split(',').map(c => c.trim()).filter(Boolean);
                tags.forEach(tag => uniqueCategories.add(tag));
            }
        });
    ...
    // Apply Filters (Search, Tag-based Categories, & Price)
    const filteredProducts = useMemo(() => {
        ...
        return products.filter(product => {
            ...
            // 2. Category Tag Match (The Fix)
            const productTags = product.category 
                ? product.category.split(',').map(c => c.trim().toLowerCase()) 
                : [];
            
            // Check if selectedCategory is "All", OR if it exists inside the product's tags
            const matchCategory = selectedCategory === "All" || productTags.includes(selectedCategory.toLowerCase());
    ...
    if (loading) {
        return (
            <div className="min-h-[60vh] flex flex-col justify-center items-center font-sans">
                ...
                <p className="mt-4 font-semibold text-[#9A9A9A]">Loading collection...</p>
            </div>
        );
    }
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-10 py-10 font-sans">
            ...
            <div className="flex justify-between items-end mb-8 border-b border-black/10 pb-6">
                ...
                <button 
                    ...
                    className="md:hidden bg-[#F0F0F0] p-3 rounded-full flex items-center justify-center cursor-pointer"
                >
            ...
            <div className="flex flex-col md:flex-row gap-8">
                ...
                `}>
                    <div className="border border-black/10 rounded-3xl p-6 bg-white sticky top-24">
                        ...
                        {/* Categories */}
                        <div className="mb-8">
                            <h3 className="font-semibold mb-4 text-[16px]">Category</h3>
                            <div className="flex flex-col gap-3 max-h-60 overflow-y-auto pr-2">
                                {categories.map(cat => (
                                    <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                                        ...
                                        />
                                        <span className={`text-[15px] transition-colors ${selectedCategory === cat ? 'font-semibold text-black' : 'text-black/60 group-hover:text-black'}`}>
                                            {cat}
                                        </span>
                                    </label>
                                ))}
                        ...
                        <div>
                            ...
                            <button 
                                ...
                                className="w-full mt-6 bg-[#F2F0F1] hover:bg-black hover:text-white text-black font-semibold rounded-full py-3 transition-colors text-sm cursor-pointer"
                            >
                                Clear Filters
                            </button>
                        </div>
                    ...
                    {filteredProducts.length === 0 ? (
                        <div className="h-64 flex flex-col justify-center items-center text-center bg-[#F2F0F1] rounded-3xl p-6">
                            ...
                            <p className="text-[#9A9A9A]">
                                Try adjusting your filters, category, or search criteria.
                            </p>
                        </div>
                    ) : (
